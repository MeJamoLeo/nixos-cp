function renderStreakCalendar(d) {
	...
	cal.forEach(c=>{if(c.ac_count===0){g+='<div class="streak-cell streak-none'+(c.date===ts2?' streak-today':'')+'"></div>';}else{const sc=ratingColor(c.max_difficulty);g+='<div class="streak-cell'+(c.date===ts2?' streak-today':'')+'" style="background:'+sc+';opacity:0.9"></div>';}});
